...
const AssignRoles = () => {
  ...
  if (currentRole !== 'Admin') return <Navigate to="/unauthorized" />;
  ...
  return (
    <Box sx={{ p: 5, backdropFilter: 'blur(5px)', backgroundColor: 'rgba(255, 255, 255, 0.8)' }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h4" sx={{ color: '#007b83' }}>Assign Roles</Typography>
        <TextField
      ...
      {loading ? (
        ...
        <TableContainer component={Paper}>
          <Table stickyHeader>
            <TableHead>
              <TableRow>
                {['name', 'email', 'roleName'].map((key) => (
                  <TableCell
                    key={key}
                    onClick={() => handleSort(key as keyof User)}
                    sx={{ cursor: 'pointer', fontWeight: 'bold', backgroundColor: '#77D5CB' }}
                  >
                    {key[0].toUpperCase() + key.slice(1)}{' '}
                    {sortConfig.key === key && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                  </TableCell>
                ))}
                <TableCell sx={{ fontWeight: 'bold', backgroundColor: '#77D5CB' }}>Action</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
                ...
                sortedUsers
                  ...
                  .map((user) => (
                    <TableRow key={user.id} hover>
                      ...
                      <TableCell>
                        <IconButton color="primary" onClick={() => { setEditUser(user); setEditRole(user.roleName ?? ''); setShowEditUserDialog(true); }}><Edit /></IconButton>
                        <IconButton color="error" onClick={() => { setSelectedUser(user); setShowDeleteUserDialog(true); }}><Delete /></IconButton>
                      </TableCell>
                    </TableRow>
                  ))
              )}
      ...
      {/* Edit User Dialog */}
      <Dialog open={showEditUserDialog} onClose={() => setShowEditUserDialog(false)} fullWidth maxWidth="sm">
        <DialogTitle>Edit Role</DialogTitle>
        <DialogContent sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
          <TextField label="Name" value={editUser?.name ?? ''} disabled fullWidth />
      ...
      {/* Delete User Dialog */}
      <Dialog open={showDeleteUserDialog} onClose={() => setShowDeleteUserDialog(false)} fullWidth maxWidth="sm">
